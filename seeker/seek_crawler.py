...
# if this mapping is outdated plz run update_classification_list()
...
def get_all_pages(BASE_URL,headers,keyword,subclassification,location,pages_to_parse,SAVE_DIR,**kwargs):
...
def xlsxWriter(outputs,fullname,expiry):    
...
def main(BASE_URL,headers,keyword,subclassification,location,pages_to_parse,SAVE_DIR,expiry):
    # scrape all pages
    outputs, fullname = get_all_pages(BASE_URL,headers,keyword,subclassification,location,pages_to_parse,SAVE_DIR)

    # write to xlsx
    xlsxWriter(outputs,fullname,expiry)
